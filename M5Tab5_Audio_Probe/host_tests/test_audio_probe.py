...
class AudioProbeHostTests(unittest.TestCase):
    def test_full_capture_stage_uses_nonblocking_recording_service(self):
        ...
        self.assertIn("M5.Mic.record(", source)
        self.assertIn("void serviceAudioCapture()", source)
    ...
    def test_touch_release_and_single_action_rendering(self):
        source = SKETCH.read_text(encoding="utf-8")
        self.assertIn("M5.Touch.getCount() == 0) {\n    touchWasDown = false", source)
        ...
        self.assertIn("kTouchReleaseMs = 30", source)
        self.assertIn("kTouchHealthReadAttempts = 3", source)
        self.assertIn("kTouchHealthQuietMs = 5000", source)
        self.assertIn("M5.delay(700)", source)
        self.assertIn("TOUCHRESET", source)
        self.assertIn("touchCriticalState", source)
        self.assertIn("storageBusy", source)
        loop_source = source[source.index("void loop()") :]
        self.assertNotIn("serviceTouchHealth();", loop_source)
        self.assertIn("periodic_health_disabled", source)
        self.assertNotIn("TOUCH_SUPPRESSED", source)
        self.assertNotIn("action=retry", source)
        self.assertIn("kInactive = TFT_DARKGREY", source)
        self.assertIn("kSelectable = 0x87F0", source)
        self.assertIn("kDetail = 0x0320", source)
        self.assertIn("TFT_WHITE, TFT_RED", source)
