...
enum class EspUsbHostOperationStatus : uint8_t
{
  Completed,
  Failed,
  Canceled,
...
struct EspUsbHostOperationToken
{
...
struct EspUsbHostOperationCompletion
{
  EspUsbHostOperationToken token;
  EspUsbHostOperationStatus status = EspUsbHostOperationStatus::OutcomeUnknown;
  uint8_t address = 0;
...
public:
  ...
  using OperationCompletionCallback = std::function<void(const EspUsbHostOperationCompletion &)>;
  ...
  void onOperationCompletion(OperationCompletionCallback callback);
  ...
  bool serialWriteAsync(const uint8_t *data,
                        ...
                        uint32_t timeoutMs = 0,
                        uint8_t address = ESP_USB_HOST_ANY_ADDRESS);
  bool serialWriteTokenized(const uint8_t *data,
                            size_t length,
                            const EspUsbHostOperationToken &token,
                            uint8_t address = ESP_USB_HOST_ANY_ADDRESS);
  bool setSerialConfigTokenized(const EspUsbHostSerialConfig &config,
                                const EspUsbHostOperationToken &token,
                                uint8_t address = ESP_USB_HOST_ANY_ADDRESS);
  bool submitVendorSerialControlTokenized(uint8_t requestType,
                                          uint8_t request,
                                          uint16_t value,
                                          uint16_t index,
                                          const EspUsbHostOperationToken &token,
...
private:
  ...
  struct DeviceState
  {
    ...
    EspUsbHostOperationToken serialOutTokens[ESP_USB_HOST_SERIAL_WRITE_QUEUE_MAX_DEPTH] = {};
    bool serialOutTokenActive[ESP_USB_HOST_SERIAL_WRITE_QUEUE_MAX_DEPTH] = {};
  ...
  OperationCompletionCallback operationCompletionCallback_;
  ...
  RetiredSerialTransfer retiredSerialTransfers_[ESP_USB_HOST_SERIAL_WRITE_QUEUE_MAX_DEPTH] = {};
  bool retiredSerialOverflow_ = false;

  void emitOperationCompletion(const EspUsbHostOperationToken &token,
